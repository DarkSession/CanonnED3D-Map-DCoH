...
import { Ico } from './icon.class';
import { Ed3d, Loader } from './ed3dmap';
...
export var HUD = {
  ...
  'initControls': function () {
    ...
    $('#controls a').click(function (e) {
      ...
      switch (view) {
        case 'top': {
          Ed3d.isTopView = true;
          var moveFrom = { x: Ed3d.camera.position.x, y: Ed3d.camera.position.y, z: Ed3d.camera.position.z };
          const moveCoords = { x: Ed3d.controls.target.x, y: Ed3d.controls.target.y + 500, z: Ed3d.controls.target.z };
        ...
        case '3d': {
          ...
          var moveFrom = { x: Ed3d.camera.position.x, y: Ed3d.camera.position.y, z: Ed3d.camera.position.z };
          const moveCoords = { x: (Ed3d.controls.target.x - 100), y: Ed3d.controls.target.y + 500, z: Ed3d.controls.target.z + 500 };
          HUD.moveCamera(moveFrom, moveCoords);
          break;
        }
        case 'infos':
  ...
  'moveCamera': function (from, to) {
    ...
    Ed3d.tween = new TWEEN.Tween(from).to(to, 800)
      .start()
      .onUpdate(function () {
        Ed3d.camera.position.set(from.x, from.y, from.z);
      })
      .onComplete(function () {
        Ed3d.controls.update();
      });
  ...
  'initHudAction': function () {
    ...
    $("canvas").hover(
      function () {
        Ed3d.controls.enabled = true;
      }, function () {
        Ed3d.controls.enabled = false;
      }
    );
    ...
    $('.map_filter').click(function (e) {
          ...
          $(Ed3d.catObjsRoutes).each(function (indexCat, listGrpRoutes) {
            if (listGrpRoutes != undefined)
              $(listGrpRoutes).each(function (key, indexRoute) {
                Ed3d.scene.getObjectByName(indexRoute).visible = false;
                if (Ed3d.scene.getObjectByName(indexRoute + '-first') != undefined)
                  Ed3d.scene.getObjectByName(indexRoute + '-first').visible = false;
                if (Ed3d.scene.getObjectByName(indexRoute + '-last') != undefined)
                  Ed3d.scene.getObjectByName(indexRoute + '-last').visible = false;
              });
          });
      ...
      if (Ed3d.catObjsRoutes.length > 0)
        $(Ed3d.catObjsRoutes[idCat]).each(function (key, indexRoute) {
          var isVisible = Ed3d.scene.getObjectByName(indexRoute).visible;
          if (isVisible == undefined) isVisible = true;
          isVisible = (isVisible ? false : true);
          Ed3d.scene.getObjectByName(indexRoute).visible = isVisible;
          if (Ed3d.scene.getObjectByName(indexRoute + '-first') != undefined)
            Ed3d.scene.getObjectByName(indexRoute + '-first').visible = isVisible;
          if (Ed3d.scene.getObjectByName(indexRoute + '-last') != undefined)
            Ed3d.scene.getObjectByName(indexRoute + '-last').visible = isVisible;
        });
  ...
  'addText': function (id, textShow, x, y, z, size, addToObj, isPoint) {
    ...
    if (addToObj == undefined) addToObj = Ed3d.scene;
  ...
  'rotateText': function (id) {
    ...
    if (Ed3d.textSel[id] != undefined)
      if (Ed3d.isTopView) {
        ...
        Ed3d.textSel[id].rotation.y = Ed3d.camera.rotation.y;
