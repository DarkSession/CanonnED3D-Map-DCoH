...
export var Route = {
    ...
    $(Ed3d.catObjsRoutes).each(function (indexCat, listGrpRoutes) {
      if (listGrpRoutes != undefined)
        ...
        $(listGrpRoutes).each(function (key, indexRoute) {
          Ed3d.scene.remove(Ed3d.scene.getObjectByName(indexRoute));
          if (Ed3d.scene.getObjectByName(indexRoute + '-first') != undefined)
            Ed3d.scene.remove(Ed3d.scene.getObjectByName(indexRoute + '-first'));
          if (Ed3d.scene.getObjectByName(indexRoute + '-last') != undefined)
            Ed3d.scene.remove(Ed3d.scene.getObjectByName(indexRoute + '-last'));
        });
    });
  },
  ...
  'createRoute': function (idRoute, route) {
    ...
    //-- Add lines to scene
    Ed3d.routes[idRoute] = new THREE.Line(geometryL, colorLine);
    ...
    Ed3d.routes[idRoute].name = 'route-' + idRoute;

    Ed3d.scene.add(Ed3d.routes[idRoute]);

    if (route.cat !== undefined) {
      $.each(route.cat, function (keyArr, idCat) {
        if (Ed3d.catObjsRoutes[idCat] == undefined)
          Ed3d.catObjsRoutes[idCat] = [];
        Ed3d.catObjsRoutes[idCat].push(Ed3d.routes[idRoute].name);
      });
    }
