...
export class Galaxy {
  ...
  public remove() {
    ...
    this.Ed3d.scene.remove(this.milkyway2D);
  }

  public addGalaxyCenter() {
    const objVal =
    {
      name: 'Sagittarius A*',
      coords: { x: this.y, y: this.y, z: this.z },
      cat: [],
    }
    ...
    var sprite = new THREE.Sprite(this.Ed3d.material.glow_2);
  ...
  public createParticles() {
    ...
    img.onload = () => {
      ...
      if (this.Ed3d.startAnim) {
        this.Ed3d.camera.position.set(-10000, 40000, 50000);
    ...
    //-- load img source
    img.src = this.Ed3d.basePath + "textures/heightmap7.jpg";
  ...
  public showGalaxyInfos() {
    if (!this.Ed3d.showGalaxyInfos) return;

    this.infos = new THREE.Object3D();

    $.getJSON(this.Ed3d.basePath + "data/milkyway-ed.json", (data) => {
      $.each(data.quadrants, (key, val) => {
        this.addText(key, val.x, -100, val.z, val.rotate);
      });
      $.each(data.arms, (key, val) => {
        $.each(val, (keyCh, valCh) => {
          this.addText(key, valCh.x, 0, valCh.z, valCh.rotate, 300, true);
        });
      });

      $.each(data.gaps, (key, val) => {
        $.each(val, (keyCh, valCh) => {
          this.addText(key, valCh.x, 0, valCh.z, valCh.rotate, 160, true);
        });
      });
      $.each(data.others, (key, val) => {
        $.each(val, (keyCh, valCh) => {
          this.addText(key, valCh.x, 0, valCh.z, valCh.rotate, 160, true);
        });
      });
    }).done(() => {
      this.Ed3d.scene.add(this.infos);
    });
  }
  ...
  public infosShow() {
    ...
    if (this.infos !== null) {
      this.infos.visible = this.Ed3d.showGalaxyInfos;
    }
  }
   ...
   */
  public infosUpdateCallback(scale) {
    if (!this.Ed3d.showGalaxyInfos || this.infos == null) {
      return;
    }
  ...
  public add2DPlane() {
    ...
    //-- Load textures
    var back2D = texloader.load(this.Ed3d.basePath + "textures/heightmap7.jpg");
    ...
    this.Ed3d.scene.add(this.milkyway2D);
  }
  ...
  public addText(textShow, x, y, z, rot, size = 450, revert = false) {
  ...
  public getHeightData(img, obj) {
    ...
    var particleMaterial = new THREE.PointsMaterial({
      map: this.Ed3d.textures.flare_yellow,
      transparent: true,
      size: 64,
